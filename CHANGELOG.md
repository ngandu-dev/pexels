# Changelog

All notable changes to this project are documented in this file. The project follows [Semantic Versioning](https://semver.org/).

## 2.0.0

### Changed

- Moved the Composer package to `ngandu-dev/pexels`.
- Adopted the `Ngandu\\Pexels` PHP namespace.
- Standardized project documentation, quality commands, and continuous integration.

### Migration

- Require `ngandu-dev/pexels` and update application imports to begin with `Ngandu\\Pexels`.
